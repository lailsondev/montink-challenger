...
class EmailService
{
    ...
    public function sendOrderConfirmationEmail($recipientEmail, $orderData, $orderItems, $status)
    {
        try {
            $this->mailer->addAddress($recipientEmail);
            $this->mailer->Subject = "Confirmação do seu Pedido #{$orderData['id']}";
            $bodyOrderStatus = "<p>Seu pedido #<strong>{$orderData['id']}</strong> foi recebido e está sendo processado.</p>";

            if (in_array($status, MSG_SEND_CONFIRMATION_ARRAY_OPTIONS)) {
                $this->mailer->Subject = "O seu Pedido #{$orderData['id']} recebeu uma atualização";
                $bodyOrderStatus = "<p>Seu pedido #<strong>{$orderData['id']}</strong> foi {$status}</p>";
            }

            $body = "<h1>Pedido #{$orderData['id']}</h1>";
